...
static uint16_t osc_ch1[OSC_SAMPLE_MAX], osc_ch2[OSC_SAMPLE_MAX]; static uint32_t osc_capture_id; static uint32_t osc_sample_rate_hz=100000u; static uint32_t osc_sample_count=512u;
...
static void osc_init(void){adc_init();adc_gpio_init(ZL_OSC_CH1_ADC_GPIO);adc_gpio_init(ZL_OSC_CH2_ADC_GPIO);adc_select_input(ZL_OSC_CH1_ADC_CHANNEL);}
static uint32_t osc_capture_now(uint32_t rate,uint32_t count){
  if(rate<1000u)rate=1000u;if(rate>200000u)rate=200000u;if(count<32u)count=32u;if(count>OSC_SAMPLE_MAX)count=OSC_SAMPLE_MAX;
  osc_sample_rate_hz=rate;osc_sample_count=count;uint32_t period_us=1000000u/rate;absolute_time_t next=get_absolute_time();
  for(uint32_t i=0;i<count;++i){
    adc_select_input(ZL_OSC_CH1_ADC_CHANNEL);osc_ch1[i]=adc_read();
    adc_select_input(ZL_OSC_CH2_ADC_CHANNEL);osc_ch2[i]=adc_read();
...
static void osc_read_channel(const wfl2_header_t*req,const uint8_t*payload,uint16_t*samples,uint8_t*out,uint32_t*out_len,uint16_t*status){
  if(req->payload_length!=12u){*status=WFL2_RANGE;return;}uint32_t id=rd32(payload),off=rd32(payload+4),count=rd32(payload+8);if(id!=osc_capture_id||off>=osc_sample_count){*status=WFL2_RANGE;return;}uint32_t avail=osc_sample_count-off;if(count>avail)count=avail;uint32_t max=(TX_FRAME_SIZE-WFL2_HEADER_SIZE-12u)/2u;if(count>max)count=max;wr32(out,id);wr32(out+4,off);wr32(out+8,count);for(uint32_t i=0;i<count;++i)wr16(out+12+i*2,samples[off+i]);*out_len=12+count*2;
}
...
case WFL2_GET_DEVICE_INFO:{size_t off=0,n;n=tlv_text(out+off,sizeof(out)-off,1,"ZooLark RP2350B Web Instrument");off+=n;n=tlv_text(out+off,sizeof(out)-off,2,"0.4.5-dual-adc-dim");off+=n;n=tlv_text(out+off,sizeof(out)-off,3,"ZL-RP2350B-DUALADC-0001");off+=n;n=tlv_text(out+off,sizeof(out)-off,4,"USB Full-Speed Vendor Bulk");off+=n;n=tlv_text(out+off,sizeof(out)-off,5,"CH1-ADC7-GPIO47,CH2-ADC5-GPIO45,KXTJ3-I2C1-GPIO22-23,WS2812-5PCT");off+=n;out_len=(uint32_t)off;break;}
case WFL2_ENTER_BOOTLOADER:enter_bootloader_pending=true;break;
case WFL2_OSC_CAPTURE:{uint32_t rate=100000u,count=512u;if(req->payload_length>=8u){rate=rd32(payload);count=rd32(payload+4);}uint32_t id=osc_capture_now(rate,count);wr32(out,id);wr32(out+4,osc_sample_rate_hz);wr32(out+8,osc_sample_count);wr16(out+12,ZL_OSC_CH1_ADC_GPIO);wr16(out+14,ZL_OSC_CH1_ADC_CHANNEL);wr16(out+16,ZL_OSC_CH2_ADC_GPIO);wr16(out+18,ZL_OSC_CH2_ADC_CHANNEL);out_len=20;break;}
case WFL2_OSC_READ:osc_read_channel(req,payload,osc_ch1,out,&out_len,&status);break;
case WFL2_OSC_READ_CH2:osc_read_channel(req,payload,osc_ch2,out,&out_len,&status);break;
