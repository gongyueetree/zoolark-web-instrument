...
static void usb_send_response(const wfl2_header_t *req, uint16_t status, const uint8_t *payload, uint32_t payload_len) {
  ...
  if (payload_len + WFL2_HEADER_SIZE > sizeof(frame)) {
    status = WFL2_RANGE; payload = NULL; payload_len = 0;
  }
...
static void handle_command(const wfl2_header_t *req, const uint8_t *payload) {
  ...
  switch ((wfl2_opcode_t)req->opcode) {
    ...
    case WFL2_ENTER_BOOTLOADER:
      enter_bootloader_pending = true;
      break;
    ...
    case WFL2_LOGIC_ARM: {
      ...
      wr32(out, s.capture_id); wr32(out + 4, s.sample_count); out_len = 8;
      break;
    }

    case WFL2_LOGIC_STATUS: {
      zl_logic_status_t s = logic_capture_status();
      out[0] = s.state; out[1] = s.error; wr16(out + 2, 0);
      wr32(out + 4, s.capture_id); wr32(out + 8, s.sample_count); wr32(out + 12, s.sample_rate_hz); out_len = 16;
      break;
    }
    ...
    case WFL2_LOGIC_READ: {
      ...
      wr32(out, capture_id); wr32(out + 4, offset); wr32(out + 8, n); out_len = 12 + n;
      break;
    }
    ...
    case WFL2_JTAG_CONFIG:
      ...
      jtag_configure(rd32(payload), rd16(payload + 4));
      break;
    ...
    case WFL2_FPGA_PROGRAM_ABORT: {
      const char *msg = "FPGA programming is not enabled in v0.4-test; JTAG scan only";
      size_t len = strlen(msg); memcpy(out, msg, len); out_len = len; status = WFL2_BAD_COMMAND;
      break;
    }
...
static void parse_rx(void) {
  while (rx_used >= WFL2_HEADER_SIZE) {
    ...
    if (h->magic == WFL2_MAGIC_U32 && h->version == WFL2_VERSION && h->type == WFL2_FRAME_COMMAND && crc == h->crc32) {
      ...
      usb_send_response(h, WFL2_BAD_CRC, NULL, 0);
    }
...
void tud_vendor_rx_cb(uint8_t itf, uint8_t const *buffer, uint16_t bufsize) {
  ...
  while (tud_vendor_available()) {
    ...
    uint32_t n = tud_vendor_read(rx_stage + rx_used, sizeof(rx_stage) - rx_used);
    rx_used += n;
    parse_rx();
  }
}
