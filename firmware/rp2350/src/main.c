...
case WFL2_GET_DEVICE_INFO:{size_t off=0,n;n=tlv_text(out+off,sizeof(out)-off,1,"ZooLark RP2350B Web Instrument");off+=n;n=tlv_text(out+off,sizeof(out)-off,2,"0.4.4-dual-adc");off+=n;n=tlv_text(out+off,sizeof(out)-off,3,"ZL-RP2350B-DUALADC-0001");off+=n;n=tlv_text(out+off,sizeof(out)-off,4,"USB Full-Speed Vendor Bulk");off+=n;n=tlv_text(out+off,sizeof(out)-off,5,"CH1-ADC7-GPIO47,CH2-ADC5-GPIO45,KXTJ3-I2C1-GPIO22-23,WS2812-20PCT");off+=n;out_len=(uint32_t)off;break;}
